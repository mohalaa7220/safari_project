...
class AddReport(generics.ListCreateAPIView):
    queryset = Report.objects.select_related('user', 'product').all()
...
def report_download(request, report_id=None):
    if report_id is not None:
        ...
        worksheet.cell(row=1, column=5, value="In Stock")
        ...
        worksheet.cell(row=2, column=5, value=report.product.in_stock)
        worksheet.cell(row=2, column=6,
        ...
        row = 2
        for report in Report.objects.select_related('user', 'product').all():
            ...
            worksheet.cell(row=row, column=5, value=report.product.in_stock)
...
class ExportExcel(views.APIView):
    ...
    def get(self, request, pk=None):
        ...
        worksheet.cell(row=1, column=5, value=report.product.in_stock)
        worksheet.cell(row=1, column=6,
                       value=report.created.strftime("%Y-%m-%d %H:%M:%S"))
